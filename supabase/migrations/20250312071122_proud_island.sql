...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    ELSE
      ...
      -- Get the minimum position for the type
      SELECT COALESCE(MIN(position), 
        CASE WHEN NEW.is_folder THEN folder_position_base ELSE document_position_base END
      ) INTO min_position
      FROM documents
      WHERE parent_id IS NOT DISTINCT FROM NEW.parent_id
      ...
      -- If moving to a position less than minimum, place at start
      IF NEW.position < min_position THEN
        NEW.position := min_position - position_increment;
      END IF;
    END IF;
